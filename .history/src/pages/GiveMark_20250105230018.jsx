...
const GiveMark = () => {
  ...
  const [marks, setMarks] = useState(''); // Default to empty
  ...
  useEffect(() => {
    const fetchSubmissionDetails = async () => {
      try {
        ...
        setSubmission(data);
        setMarks(data.obtainedMarks); // Set the existing marks
        setFeedback(data.feedback); // Set the existing feedback
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/pending-assignments'); // Redirect to pending assignments page
      } else {
